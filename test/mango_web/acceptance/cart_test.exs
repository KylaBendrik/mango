defmodule MangoWeb.Acceptance.CartTest do
  ...
  test "presence of cart form for each product" do
    ...
    |> Enum.each(fn(product) ->
      button = fine_within_element(product, :tag, "button")
  ...
  test "add to cart" do
   ...
   [product | test] = find_all_elements(:css, ".product-card")
   
   product_name = find_withing_element(product, :name, "cart[product_name")
   ...
   assert message = "Product added to cart - #{product_name}(#{pack_size}) x 2qty"
  end
end
